...
const PublicBookingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-950 font-sans text-gray-100 selection:bg-purple-500 selection:text-white pb-12">
      ...
      {/* Background Ambience - Otimizado: Oculto no mobile */}
            ...
            <div className="animate-fade-in-up">
                ...
                <div className="bg-gray-900/60 backdrop-blur-md p-6 rounded-xl shadow-lg border border-white/5 mb-6">
                    ...
                    <div className="flex space-x-2 overflow-x-auto pb-2 custom-scrollbar">
                        {/* 56 Days List (8 Weeks) */}
                        {Array.from({length: 56}).map((_, i) => {
                            const date = addDays(startOfToday(), i); 
                            const isSelected = format(date, 'yyyy-MM-dd') === format(selectedDate, 'yyyy-MM-dd');
                            const blocked = isDateBlocked(date);
                            return (
                                <button key={i} disabled={blocked} onClick={() => { setSelectedDate(date); setSelectedTime(''); }} className={`flex-shrink-0 w-16 h-20 rounded-xl flex flex-col items-center justify-center border transition-all ${blocked ? 'bg-gray-800/50 opacity-30 cursor-not-allowed border-transparent' : isSelected ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-gray-800 border-white/5 text-gray-400 hover:bg-gray-700 hover:text-white'}`}>
                    ...
                    {/* Display selected date full text */}
                    <div className="mt-2 text-center text-sm font-bold text-primary animate-fade-in">
