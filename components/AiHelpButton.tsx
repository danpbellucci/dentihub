...
const AiHelpButton: React.FC = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ask-ai-help', {
        body: { 
          history: messages.slice(-6), // Envia apenas as últimas 6 mensagens para contexto
          message: userMsg 
        }
      });
      ...
      console.error(err);
      setMessages(prev => [...prev, { role: 'ai', content: 'Desculpe, tive um erro de conexão. Tente novamente.' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div 
        className={`fixed bottom-24 right-6 z-50 w-80 sm:w-96 bg-gray-900 border border-white/10 rounded-2xl shadow-2xl flex flex-col transition-all duration-300 origin-bottom-right overflow-hidden ${
